...
certain disparities exist. The median house price is 739,900, while the average is much higher,
960,246.46. There exists an astounding variance between the minimum and maximum value
(5,519,000.00) and number of days on the market. Since such a large range between data points
for days on the market, other variables of interest will be examined further in the paper to
...
library(readr)
library(kableExtra)
table3 <- read.csv("Figure1.csv")
knitr::kable(table3, col.names = c('Location and Dispersion', 'Price', 'Days on Market', 'Year Built'), align = "c")%>%
...
```{r fig1, fig.height = 8, fig.width = 10, echo=FALSE}
library(ggplot2)
knitr::opts_chunk$set(fig.width = 30, fig.height = 8) 
options(repr.plot.width = 4, repr.plot.height = 8)
ggplot(data = yearbuilt[1:2], aes(x = Year_Built, y = Price, group = 3))+
  expand_limits(x = 2025, y = 6000)+
...
```{r, echo=FALSE}
library(readr)
daysonmarket = read.csv("Days_On_Market_vs_Price.csv")
```

```{r fig2, fig.height = 8, fig.width = 10, echo=FALSE}
library(ggplot2)
knitr::opts_chunk$set(fig.width = 30, fig.height = 8) 
options(repr.plot.width = 4, repr.plot.height = 8)
ggplot(data = daysonmarket[1:2], aes(x = DaysMarket, y = Price, group = 3))+
  expand_limits(x = 400, y = 6000)+
  geom_point(color = "#2a63ff", size = 2, alpha = 0.9)+
  theme_bw()+
  theme(axis.text = element_text(size = 14),
        axis.title = element_text(size = 14,face = "bold"))+
  labs(title = "Enrolled",
       x = "\nDays On Market",
       y = "Price (in thousands) \n")+
  ggtitle("Days on Market vs. Price")+
  theme(plot.title = element_text(size = 18, face = "bold"))
```
...
$$
\begin{align*}
=\beta_0 + \beta_1Year\_Built\begin{pmatrix}
    1 = (1911 - 1932)\\
    2 = (1933 - 1954)\\
    3 = (1955 - 1976)\\
    4 = (1977 - 1998)\\
