...
If the equation is manipulated to $\omega = \frac{p}{0} {,}$ then the risk variable is undefined.^[$\displaystyle \lim_{x \to +\infty} \left( \frac{1}{1+r} \right)= 0$]
...
The investor wishes to maximize his gain subject to the potency *p* of the investment. This is equal to the standard deviation ($\sigma$) times the risk *(r)*, plus the reward *(R)* multiplied by depreciation $(\delta){.}$ With any investment, there exists a possibility of less than perfect reward, so we account for the loss on an investment by using the depreciation variable $(\delta){.}$
...
table1 <- knitr::kable(table1, col.names = c('', '2005', '1 month', '3 months', '12 months', 'YTD'),  align ='c')%>%
...
add_header_above(table1, c('', 'As of June 30, 2006' = 5))%>%
row_spec(0:0, bold = T, color = "black", background = "#c0c0c0")%>%
...
An uninformed investor would look at these trends and assume market degradation and/or failure from the data alone. There is enough evidence to support the claim that this particular bond market was on a steady decline, but not enough to support any claims of market
